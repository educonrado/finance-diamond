...
<template>
  <div class="flex h-screen">
    ...
    <aside
      ...
      <nav class="mt-8">
        ...
        >
          ...
          <span v-if="!isSidebarCollapsed">Préstamos</span>
        </router-link>
      </nav>
      <div
        v-if="user"
    ...
    >
      ...
      <div class="absolute inset-0 rounded-full bg-white opacity-0 group-hover:opacity-10 transition-opacity duration-200"></div>
    </button>
    <Transition enter-active-class="transition-all duration-300 ease-out"
      ...
      <div v-if="isFabMenuOpen" class="fixed bottom-24 right-6 flex flex-col items-end gap-3 z-50">
        <button @click="goToNewTransfer"
          ...
          <div class="w-8 h-8 bg-green-100 dark:bg-green-900 text-green-600 dark:text-green-400 rounded-full flex items-center justify-center group-hover:bg-green-200 dark:group-hover:bg-green-800 transition-colors">
        ...
        <button @click="goToNewTransaction"
          ...
          <div class="w-8 h-8 bg-blue-100 dark:bg-blue-900 text-blue-600 dark:text-blue-400 rounded-full flex items-center justify-center group-hover:bg-blue-200 dark:group-hover:bg-blue-800 transition-colors">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6" />
            </svg>
          </div>
